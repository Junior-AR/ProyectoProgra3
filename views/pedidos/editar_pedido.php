...
<nav class="navbar navbar-expand-lg navbar-dark">
    ...
    <div class="container">
        <div class="card p-4 mx-auto col-md-8">
            <h2 class="card-title">Editar Pedido</h2>
            <?php
            include __DIR__ . '/../../includes/conexion.php';
            include '../../Controllers/controlador_pedidos.php';

            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                $pedidoID = $_GET['id']; // Obtener el ID del pedido de la URL
                $productoID = $_POST['productoID'];
                $proveedorID = $_POST['proveedorID'];
                $fecha = $_POST['fecha'];
                $estado = $_POST['estado'];

                $editar = editarPedido($pedidoID, $productoID, $proveedorID, $fecha, $estado);
            ...
            <form method="post">
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="productoID" name="productoID" value="<?php echo $pedido['ID_Producto']; ?>" required>
                </div>

                <div class="mb-3">
                    <label for="proveedorID" class="form-label">ID Proveedor:</label>
                    <input type="number" class="form-control" id="proveedorID" name="proveedorID" value="<?php echo $pedido['ID_Proveedor']; ?>" required>
                </div>

                <div class="mb-3">
                    <label for="fecha" class="form-label">Fecha:</label>
                    <input type="date" class="form-control" id="fecha" name="fecha" value="<?php echo $pedido['FechaPedido']; ?>" required>
                </div>

                <div class="mb-3">
                    <label for="estado" class="form-label">Estado:</label>
                    <input type="text" class="form-control" id="estado" name="estado" value="<?php echo $pedido['Estado']; ?>" required>
                </div>
